#!/bin/bash

# BankBrain Cloud Shell Setup and Deploy
# This script creates the project structure and deploys BankBrain

set -e

echo "🚀 BankBrain Cloud Shell Setup and Deploy"
echo "=========================================="
echo ""

# Get project ID
PROJECT_ID=$(gcloud config get-value project)
if [ -z "$PROJECT_ID" ]; then
    echo "❌ No project ID found. Please set your project:"
    echo "   gcloud config set project YOUR_PROJECT_ID"
    exit 1
fi

echo "📋 Project ID: $PROJECT_ID"
echo "🏷️  Tag: v1"
echo ""

# Create project structure
echo "📁 Creating project structure..."
mkdir -p bankbrain_project
cd bankbrain_project
mkdir -p agents/support-agent agents/risk-agent agents/a2a-gateway mcp/bank_mcp_server k8s common

# Create basic Dockerfiles
echo "🐳 Creating Dockerfiles..."

# Support Agent Dockerfile
cat > agents/support-agent/Dockerfile << 'EOF'
FROM python:3.9-slim
WORKDIR /app
COPY requirements.txt .
RUN pip install -r requirements.txt
COPY . .
EXPOSE 8080
CMD ["python", "app.py"]
EOF

# Risk Agent Dockerfile
cat > agents/risk-agent/Dockerfile << 'EOF'
FROM python:3.9-slim
WORKDIR /app
COPY requirements.txt .
RUN pip install -r requirements.txt
COPY . .
CMD ["python", "app.py"]
EOF

# MCP Server Dockerfile
cat > mcp/bank_mcp_server/Dockerfile << 'EOF'
FROM python:3.9-slim
WORKDIR /app
COPY requirements.txt .
RUN pip install -r requirements.txt
COPY . .
EXPOSE 8000
CMD ["python", "server.py"]
EOF

# A2A Gateway Dockerfile
cat > agents/a2a-gateway/Dockerfile << 'EOF'
FROM python:3.9-slim
WORKDIR /app
COPY requirements.txt .
RUN pip install -r requirements.txt
COPY . .
EXPOSE 7000
CMD ["python", "app.py"]
EOF

# Create basic requirements.txt files
echo "📦 Creating requirements.txt files..."

cat > agents/support-agent/requirements.txt << 'EOF'
flask==2.0.1
requests==2.26.0
EOF

cat > agents/risk-agent/requirements.txt << 'EOF'
flask==2.0.1
requests==2.26.0
EOF

cat > mcp/bank_mcp_server/requirements.txt << 'EOF'
flask==2.0.1
requests==2.26.0
EOF

cat > agents/a2a-gateway/requirements.txt << 'EOF'
flask==2.0.1
requests==2.26.0
EOF

# Create basic app files
echo "📝 Creating basic app files..."

cat > agents/support-agent/app.py << 'EOF'
from flask import Flask, request, jsonify
app = Flask(__name__)

@app.route('/')
def home():
    return jsonify({"message": "BankBrain Support Agent", "status": "running"})

@app.route('/health')
def health():
    return jsonify({"status": "healthy"})

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=8080)
EOF

cat > agents/risk-agent/app.py << 'EOF'
from flask import Flask, request, jsonify
app = Flask(__name__)

@app.route('/')
def home():
    return jsonify({"message": "BankBrain Risk Agent", "status": "running"})

@app.route('/health')
def health():
    return jsonify({"status": "healthy"})

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=8080)
EOF

cat > mcp/bank_mcp_server/server.py << 'EOF'
from flask import Flask, request, jsonify
app = Flask(__name__)

@app.route('/')
def home():
    return jsonify({"message": "BankBrain MCP Server", "status": "running"})

@app.route('/health')
def health():
    return jsonify({"status": "healthy"})

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=8000)
EOF

cat > agents/a2a-gateway/app.py << 'EOF'
from flask import Flask, request, jsonify
app = Flask(__name__)

@app.route('/')
def home():
    return jsonify({"message": "BankBrain A2A Gateway", "status": "running"})

@app.route('/health')
def health():
    return jsonify({"status": "healthy"})

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=7000)
EOF

# Create Kubernetes manifests
echo "⚙️  Creating Kubernetes manifests..."

# Namespace
cat > k8s/namespace.yaml << 'EOF'
apiVersion: v1
kind: Namespace
metadata:
  name: bankbrain
EOF

# ConfigMap
cat > k8s/configmap.yaml << 'EOF'
apiVersion: v1
kind: ConfigMap
metadata:
  name: bankbrain-config
  namespace: bankbrain
data:
  ENVIRONMENT: "production"
EOF

# Support Agent
cat > k8s/support-agent.yaml << 'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: support-agent
  namespace: bankbrain
spec:
  replicas: 1
  selector:
    matchLabels: { app: support-agent }
  template:
    metadata:
      labels: { app: support-agent }
    spec:
      containers:
        - name: app
          image: REPLACE_WITH_REGISTRY/bankbrain-support-agent:REPLACE_TAG
          envFrom:
            - configMapRef: { name: bankbrain-config }
          ports:
            - containerPort: 8080
---
apiVersion: v1
kind: Service
metadata:
  name: support-agent
  namespace: bankbrain
spec:
  selector: { app: support-agent }
  ports:
    - name: http
      port: 80
      targetPort: 8080
---
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: support-agent-ingress
  namespace: bankbrain
  annotations:
    kubernetes.io/ingress.class: "gce"
spec:
  rules:
    - host: "*"
      http:
        paths:
          - path: /
            pathType: Prefix
            backend:
              service:
                name: support-agent
                port:
                  number: 80
EOF

# Risk Agent
cat > k8s/risk-agent.yaml << 'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: risk-agent
  namespace: bankbrain
spec:
  replicas: 1
  selector:
    matchLabels: { app: risk-agent }
  template:
    metadata:
      labels: { app: risk-agent }
    spec:
      containers:
        - name: worker
          image: REPLACE_WITH_REGISTRY/bankbrain-risk-agent:REPLACE_TAG
          envFrom:
            - configMapRef: { name: bankbrain-config }
          args: ["--poll-interval=30s"]
EOF

# MCP Server
cat > k8s/mcp-bank-server.yaml << 'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: mcp-bank-server
  namespace: bankbrain
spec:
  replicas: 1
  selector:
    matchLabels: { app: mcp-bank-server }
  template:
    metadata:
      labels: { app: mcp-bank-server }
    spec:
      containers:
        - name: server
          image: REPLACE_WITH_REGISTRY/bankbrain-mcp-server:REPLACE_TAG
          envFrom:
            - configMapRef: { name: bankbrain-config }
          ports:
            - containerPort: 8000
---
apiVersion: v1
kind: Service
metadata:
  name: mcp-bank-server
  namespace: bankbrain
spec:
  selector: { app: mcp-bank-server }
  ports:
    - name: http
      port: 80
      targetPort: 8000
EOF

# A2A Gateway
cat > k8s/a2a-gateway.yaml << 'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: a2a-gateway
  namespace: bankbrain
spec:
  replicas: 1
  selector:
    matchLabels: { app: a2a-gateway }
  template:
    metadata:
      labels: { app: a2a-gateway }
    spec:
      containers:
        - name: app
          image: REPLACE_WITH_REGISTRY/bankbrain-a2a-gateway:REPLACE_TAG
          ports:
            - containerPort: 7000
---
apiVersion: v1
kind: Service
metadata:
  name: a2a-gateway
  namespace: bankbrain
spec:
  selector: { app: a2a-gateway }
  ports:
    - name: http
      port: 80
      targetPort: 7000
EOF

echo "✅ Project structure created successfully!"
echo ""

# Confirm deployment
read -p "Continue with deployment? (y/N): " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "Setup completed. You can deploy later with the deployment script."
    exit 0
fi

echo ""
echo "🔐 Setting up Google Cloud..."

# Enable required APIs
echo "Enabling required APIs..."
gcloud services enable container.googleapis.com
gcloud services enable containerregistry.googleapis.com

# Create GKE cluster if it doesn't exist
CLUSTER_EXISTS=$(gcloud container clusters list --filter="name:bankbrain-cluster" --format="value(name)" 2>/dev/null || echo "")

if [ -z "$CLUSTER_EXISTS" ]; then
    echo "🏗️  Creating GKE cluster 'bankbrain-cluster'..."
    echo "This may take 5-10 minutes..."
    
    gcloud container clusters create bankbrain-cluster \
        --zone us-central1-a \
        --num-nodes 3 \
        --machine-type e2-standard-2 \
        --enable-autoscaling \
        --min-nodes 1 \
        --max-nodes 5
    
    echo "✅ Cluster created successfully!"
else
    echo "✅ Cluster 'bankbrain-cluster' already exists"
fi

# Get cluster credentials
echo "🔑 Getting cluster credentials..."
gcloud container clusters get-credentials bankbrain-cluster --zone us-central1-a

echo ""
echo "🐳 Building and pushing Docker images..."

# Build and push Support Agent
echo "Building Support Agent..."
docker build -t gcr.io/$PROJECT_ID/support-agent:v1 ./agents/support-agent
docker push gcr.io/$PROJECT_ID/support-agent:v1

# Build and push Risk Agent
echo "Building Risk Agent..."
docker build -t gcr.io/$PROJECT_ID/risk-agent:v1 ./agents/risk-agent
docker push gcr.io/$PROJECT_ID/risk-agent:v1

# Build and push MCP Server
echo "Building MCP Server..."
docker build -t gcr.io/$PROJECT_ID/mcp-server:v1 ./mcp/bank_mcp_server
docker push gcr.io/$PROJECT_ID/mcp-server:v1

# Build and push A2A Gateway
echo "Building A2A Gateway..."
docker build -t gcr.io/$PROJECT_ID/a2a-gateway:v1 ./agents/a2a-gateway
docker push gcr.io/$PROJECT_ID/a2a-gateway:v1

echo "✅ All images built and pushed successfully!"
echo ""

echo "📝 Updating Kubernetes manifests..."

# Update manifests with correct image references
sed -i "s|REPLACE_WITH_REGISTRY/bankbrain-support-agent:REPLACE_TAG|gcr.io/$PROJECT_ID/support-agent:v1|g" k8s/support-agent.yaml
sed -i "s|REPLACE_WITH_REGISTRY/bankbrain-risk-agent:REPLACE_TAG|gcr.io/$PROJECT_ID/risk-agent:v1|g" k8s/risk-agent.yaml
sed -i "s|REPLACE_WITH_REGISTRY/bankbrain-mcp-server:REPLACE_TAG|gcr.io/$PROJECT_ID/mcp-server:v1|g" k8s/mcp-bank-server.yaml
sed -i "s|REPLACE_WITH_REGISTRY/bankbrain-a2a-gateway:REPLACE_TAG|gcr.io/$PROJECT_ID/a2a-gateway:v1|g" k8s/a2a-gateway.yaml

echo "✅ Manifests updated!"
echo ""

echo "🚀 Deploying to GKE..."
kubectl apply -f k8s/

echo "✅ Deployment successful!"
echo ""

echo "⏳ Waiting for pods to be ready..."
kubectl wait --for=condition=ready pod -l app=support-agent -n bankbrain --timeout=300s
kubectl wait --for=condition=ready pod -l app=risk-agent -n bankbrain --timeout=300s
kubectl wait --for=condition=ready pod -l app=mcp-bank-server -n bankbrain --timeout=300s
kubectl wait --for=condition=ready pod -l app=a2a-gateway -n bankbrain --timeout=300s

echo "✅ All pods are ready!"
echo ""

echo "📊 Deployment Status:"
kubectl get pods -n bankbrain
echo ""

echo "🌐 Services:"
kubectl get svc -n bankbrain
echo ""

echo "🚪 Ingress:"
kubectl get ingress -n bankbrain
echo ""

echo "🎯 Your BankBrain application is now deployed!"
echo ""

# Get the public URL
echo "🔗 Getting your public URL..."
INGRESS_ADDRESS=$(kubectl get ingress -n bankbrain -o jsonpath='{.items[0].status.loadBalancer.ingress[0].ip}' 2>/dev/null)

if [ -n "$INGRESS_ADDRESS" ]; then
    echo "✅ Public URL: http://$INGRESS_ADDRESS"
    echo ""
    echo "🌐 You can now access BankBrain at:"
    echo "   http://$INGRESS_ADDRESS"
    echo ""
    echo "📱 This URL is accessible from anywhere on the internet!"
else
    echo "⏳ Ingress is still provisioning. Check status with:"
    echo "   kubectl get ingress -n bankbrain"
    echo ""
    echo "💡 You can also use port forwarding for local testing:"
    echo "   kubectl port-forward svc/support-agent 8080:80 -n bankbrain"
    echo "   Then open: http://localhost:8080"
fi

echo ""
echo "🎉 Deployment completed successfully!"
echo ""
echo "📋 Useful commands:"
echo "   Check status: kubectl get all -n bankbrain"
echo "   View logs: kubectl logs -f deployment/support-agent -n bankbrain"
echo "   Port forward: kubectl port-forward svc/support-agent 8080:80 -n bankbrain"
